...
use crate::{ModelArchitecture, ModelConfig, ModelConfigState, PayloadModelConfig, PayloadId};
    ...
    let on_click_load_unload_model = move |ev| {
        ...
        if event_target_checked(&ev) {
            spawn_local(async move {
                ...
                match tauri::invoke::<_, String>("load_dynamic_model", &()).await {
                    Ok(msg) => {
    ...
    view! { cx,
        ...
        <div class="flex-0 flex flex-row justify-between border border-gray-700 rounded-lg m-2 p-2">
            ...
            <input
                ...
                disabled=move || !model_config_loaded().0
                on:change=on_click_load_unload_model
                checked=is_model_connected
            />
        </div>
        // Model Path
        <div class="flex flex-col flex-0 border border-gray-700 rounded-lg m-2 p-2">
            ...
            >
                <button class="flex-1 flex flex-row" required=true on:click=on_click_open_file>
                    <div class="btn">
        ...
        // Availible models
        <div class="flex-1 overflow-scroll border border-gray-700 rounded-lg m-2 p-2">
                            ...
                            view=move |cx, model: ModelConfig| {
                                ...
                                view! { cx,
                                    <tr>
                                        ...
                                        <th>
                                            {format!(
                                                "{}", model.model_path.clone().into_os_string().into_string()
                                                .unwrap_or_default()
                                            )}
                                        </th>
